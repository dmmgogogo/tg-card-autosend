...
func handleText(api *tgbotapi.BotAPI, message *tgbotapi.Message) {
	...
	case 3: // 输入群组ID
		...
		// 保存配置成功后，重启机器人
		config.ID = configId
		if err := RestartBot(config); err != nil {
...
func handleDelCommand(api *tgbotapi.BotAPI, chatID int64, args string) {
	botUniqueId := strings.TrimSpace(args)
	if botUniqueId == "" {
	...
	db := orm.NewOrm()
	if _, err := db.Raw("DELETE FROM bots WHERE id = ?", botUniqueId).Exec(); err != nil {
		log.Printf("删除机器人失败：%v", err)
		sendMessage(api, chatID, "❌ 删除失败")
		return
	}

	sendMessage(api, chatID, fmt.Sprintf("✅ 成功删除机器人：`%s`", botUniqueId))

	// 删除成功后，重启所有机器人
	botUniqueIdInt, err := strconv.ParseInt(botUniqueId, 10, 64)
	if err != nil {
		log.Printf("转换机器人ID失败：%v", err)
		sendMessage(api, chatID, "❌ 转换机器人ID失败")
		return
	}

	StopBot(botUniqueIdInt)
	sendMessage(api, chatID, "✨ 所有机器人已重新启动")
}

func handleListCommand(api *tgbotapi.BotAPI, chatID int64) {
	db := orm.NewOrm()
	configs := make([]*models.Bot, 0)
	_, err := db.QueryTable("bot").All(&configs)
	if err != nil {
